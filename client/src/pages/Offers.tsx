...
type SortKey = "maxClicks" | "date" | "duration" | "textBudget" | "ripsBudget" | "ctr" | "viewed";
...
export default function Offers() {
  ...
  const handleApplyBudgets = async (config: { 
    ...
    try {
      ...
      const updatePromises = targetOffers.map(async (offer) => {
        return apiRequest("PATCH", `/api/offers/${offer.id}`, {
          textBudgetDollars: config.textBudget || offer.textBudgetDollars,
          ripsBudgetDollars: config.ripsBudget || offer.ripsBudgetDollars,
          maxClicksAllowed: config.maxClicks || offer.maxClicksAllowed,
      ...
      toast({
        title: "Update Failed",
        description: error.message || 'Failed to update budgets. Please try again.',
        variant: "destructive",
      });
    } finally {
      setIsApplyingQuickInputs(false);
  ...
  const renderView = () => {
    // Stage 1: Show all offers needing configuration (batch + single drafts)
    if (activeView === "folder1") {
      // Get all Stage 1 offers (batch offers + single draft offers)
      let allStage1Offers: Offer[] = [...categorizedData.stage1Offers];
      
      console.log("📁 Stage 1 View Debug:");
      console.log(`   Total Stage 1 offers: ${allStage1Offers.length}`);
      console.log(`   - Batch offers: ${categorizedData.stage1Offers.filter(o => o.batchPendingSelection).length}`);
      console.log(`   - Single draft offers: ${categorizedData.stage1Offers.filter(o => !o.batchPendingSelection).length}`);
      console.log(`   Has active filters: ${hasActiveFilters}`);
      
      // Apply filters if active
      if (hasActiveFilters) {
        allStage1Offers = allStage1Offers.filter(offer => filteredOffers.includes(offer));
      }
      
      // Apply sorting
      allStage1Offers.sort((a, b) => {
        let comparison = 0;
        
        switch (sortKey) {
          case "maxClicks":
            comparison = (a.maxClicksAllowed || 0) - (b.maxClicksAllowed || 0);
            break;
          case "date":
      ...
      console.log(`   Final Stage 1 offer count: ${allStage1Offers.length}`);
      
      // Calculate total and filtered counts
      const totalStage1Count = categorizedData.stage1Offers.length;
      const filteredStage1Count = hasActiveFilters ? allStage1Offers.length : totalStage1Count;
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6 items-start">
            ...
            <div className="space-y-4">
              ...
              {/* Quick Setup - Moved down further */}
              <div className="mt-48">
                <Card>
                  <CardContent className="pt-6 pb-4">
                    <QuickInputCard 
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <Button
                  ...
                  disabled={allStage1Offers.length === 0 || deleteAllMutation.isPending}
                  data-testid="button-delete-all-stage1"
                >
              ...
              <Card className="bg-blue-50 dark:bg-blue-950/20">
                ...
                {allStage1Offers.length === 0 ? (
                  <div className="text-center py-12">
                    <Folder className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
                    <h3 className="text-lg font-semibold mb-2">No Offers</h3>
                    <p className="text-muted-foreground">
                      {hasActiveFilters ? 'No offers match your filters' : 'Create single offers or batch permutations to configure them here'}
                    </p>
                  </div>
                ) : (
                  <>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <Button
                  ...
                  data-testid="button-delete-all-stage2"
                >
                  <Trash2 className="h-4 w-4 mr-2" />
                  {deleteAllMutation.isPending ? "Deleting..." : "Delete All"}
                </Button>
